...
/**
 * Хендлер легкого benchmark-теста.
 *
 * @remarks
 ...
 */
@injectable()
export class LightBenchmarkHandler extends BaseBenchmarkHandler {
  /** Тип benchmark-теста. */
  public readonly mode: BenchmarkTestType = BenchmarkTestType.Light;

  /** Название группы тестовых объектов. */
  protected readonly groupName = '__LIGHT_BENCHMARK__';
  ...
  protected fillGroup(group: THREE.Group): void {
    ...
    for (let i = 0; i < this.objectsCount; i += 1) {
      ...
      mesh.position.set(x - offset, 0.2, z - offset);
